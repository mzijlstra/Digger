...
void openplay(char *name) {
    ...
    char *res; // used to supress warning: fgets result ingnored
#ifdef INTDRF
    ...
    res = fgets(buf, 80, playf); /* Get id string */
    if (res && (buf[0] != 'D' || buf[1] != 'R' || buf[2] != 'F')) {
        fclose(playf);
        escape = TRUE;
        return;
    }
    res = fgets(buf, 80, playf); /* Get version for kludge switches */
    if (atol(buf + 7) <= 19981125l)
        kludge = TRUE;
    res= fgets(buf, 80, playf); /* Get mode */
    if (*buf == '1') {
        nplayers = 1;
        x = 1;
    } else
    ...
    res = fgets(buf, 80, playf); /* Get bonus score */
    bonusscore = atoi(buf);
    for (n = 0; n < 8; n++)
        for (y = 0; y < 10; y++) {
            for (x = 0; x < 15; x++)
                buf[x] = ' ';
            res = fgets(buf, 80, playf); /* Get a line of map */
            for (x = 0; x < 15; x++)
