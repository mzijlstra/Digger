...
Uint sound_device, sound_port, sound_irq, sound_dma, sound_rate, sound_length;

void parsecmd(int argc, char *argv[]) {
    char *word;
    Sint4 arg, i = 0, j = 0, speedmul = 0;
    ...
    if (levfflag) {
        ...
        if (levf == NULL)
            ...
            if (!fread(&bonusscore, 2, 1, levf) ||
                    !fread(leveldat, 1200, 1, levf)) {
                fprintf(stderr, "Unable to read %s", levfname);
            }
            fclose(levf);
        }
    }
