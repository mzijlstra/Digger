...
void WriteINIString(char *section, char *key, char *value, char *filename) {
    ...
    ret = fread(buffer, tl, 1, fp);
    if (ret != 1 && (ferror(fp) || feof(fp))) {
        free(buffer);
        fclose(fp);
        return;
    }
...
void GetINIString(char *section, char *key, char *def, char *dest,
    ...
    char *ret; // to suppress warnings for fgets return
    FILE *fp;
    ...
    do {
        ret = fgets(s1, 80, fp);
        sgets(s1, s1);
        if (ret && stricmp(s1, s2) == 0) {
            do {
                ret = fgets(s1, 80, fp);
                sgets(s1, s1);
                if (ret && strnicmp(s1, s3, strlen(s3)) == 0) {
